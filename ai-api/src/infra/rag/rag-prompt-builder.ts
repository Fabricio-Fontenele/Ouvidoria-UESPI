...
export class RagPromptBuilder {
  build({ history, message, userRole, contextChunks, catalog }: BuildPromptInput): {
    ...
    const systemPrompt = [
      ...
      '8. Use EXATAMENTE os ids do CATÁLOGO para preencher `campusId` e `administrativeUnitId`. INFIRA o `administrativeUnitId` SEMPRE que o relato mencionar curso, setor, profissional, espaço físico ou contexto suficiente — mesmo se o usuário disser "não sei", VOCÊ resolve a partir do contexto, não devolve a pergunta. Heurística obrigatória (use nessa ordem):',
      '   (i) Se há um CURSO mencionado (ex.: "aluno/professor de computação", "matrícula em pedagogia") → procure no catálogo do campus uma unidade cujo `label` contenha o nome do curso ("Computação", "Ciência da Computação", "Pedagogia", etc.). Costuma ser "Coordenação do Curso de X" ou "Curso de X".',
      '   (ii) Se há um SETOR mencionado ("secretaria", "biblioteca", "restaurante/RU", "PRAD", "PROPLAN", "TI", "portaria") → match direto pelo `label` da unidade no catálogo.',
      '   (iii) Se é INCIDENTE entre alunos / assédio / agressão sem setor específico, mas com curso → use "Coordenação do Curso de X" (do curso do envolvido). Se também não houver curso → use "Direção do Campus" ou equivalente do campus em questão.',
      '   (iv) Se é problema "geral do campus" (estrutura, segurança patrimonial) → "Direção do Campus" / "Diretoria" do campus.',
      '   (v) Só depois de exaurir (i)–(iv) é que pode ficar `null` — e nesse caso, NÃO pergunte ao usuário a unidade exata: ofereça 2-3 opções concretas do catálogo do campus ("seria mais a Direção, a Coordenação de Computação, ou a Secretaria Acadêmica?"). Nunca peça "qual unidade?" sem opções.',
