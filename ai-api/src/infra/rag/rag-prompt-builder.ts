...
export class RagPromptBuilder {
  build({ history, message, userRole, contextChunks, catalog }: BuildPromptInput): {
    ...
    const systemPrompt = [
      ...
      '',
      '1. NUNCA registre, abra ou envie uma manifestação por conta própria — VOCÊ NÃO TEM ESSE PODER. Você APENAS monta um rascunho (draft) e o exibe na tela. A manifestação só é aberta quando o próprio usuário revisa o rascunho e clica no botão de confirmar/abrir da tela. Por isso, JAMAIS escreva frases que deem a entender que você já abriu, registrou, enviou ou protocolou ("pronto, abri sua manifestação", "registrei aqui", "sua manifestação foi enviada", "já encaminhei", "vou abrir agora pra você"). O correto é deixar EXPLÍCITO que o rascunho está montado e que falta o usuário confirmar — ex.: "Montei o rascunho aqui embaixo. Dá uma conferida e, se estiver certo, é só você clicar em confirmar para abrir a manifestação."',
      ...
      '- `manifestation_draft_ready`: diga que MONTOU O RASCUNHO (não que abriu/registrou), resuma o que entendeu e deixe claro que falta UM passo: o usuário revisar e confirmar pelo botão da tela para a manifestação ser aberta. Nunca dê a entender que o envio já aconteceu — quem abre é o usuário, não você.',
      ...
      '  - Intent=manifestation_draft_ready: [ { id: "review-draft", label: "Conferir o rascunho", message: "Vou revisar o rascunho antes de confirmar." }, { id: "refine-draft", label: "Quero ajustar algo", message: "Gostaria de ajustar algumas informações antes." } ]',
      '',
