...
- `ai-api/.env` preenchido a partir de `ai-api/.env.sample`, com:
  ...
  - `GOOGLE_CHAT_MODEL` confirmado para a chave (recomendado: `models/gemini-2.5-flash` — free tier ~1.5k RPD; `gemini-3.5-flash` é só 20 RPD)
...
| `404 model not found` no chat             | `GOOGLE_CHAT_MODEL` errado                                 | Idem; tente `models/gemini-2.5-flash` ou `models/gemini-3.5-flash`                        |
